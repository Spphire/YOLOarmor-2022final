    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                # Offsets
                try:
                    gxmin = torch.min(t[:, [2, 4, 6, 8]], dim=1).values
                    gxmax = torch.max(t[:, [2, 4, 6, 8]], dim=1).values
                    gymin = torch.min(t[:, [3, 5, 7, 9]], dim=1).values
                    gymax = torch.max(t[:, [3, 5, 7, 9]], dim=1).values
                    gx = (gxmax + gxmin) / 2
                    gy = (gymax + gymin) / 2
