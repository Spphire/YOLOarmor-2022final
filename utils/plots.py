...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    cv2.line(img, c1, c2, color, thickness=1, lineType=cv2.LINE_AA)
    cv2.line(img, c2, c3, color, thickness=1, lineType=cv2.LINE_AA)
    cv2.line(img, c3, c4, color, thickness=1, lineType=cv2.LINE_AA)
    cv2.line(img, c4, c1, color, thickness=1, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        x1 = min(c1[0],c2[0],c3[0],c4[0])
        y1 = min(c1[1],c2[1],c3[1],c4[1])
        c1 = x1, y1
